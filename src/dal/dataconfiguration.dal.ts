...
export default class DataCofigurationDAL {
  async getCampaigns() {
      ...
      const camnpaigns:any = await mysqlclient.executeQuery(sql);
      if (camnpaigns != null && camnpaigns.length > 0) {
          const campaignData = await this.constructUserDataFromUser(camnpaigns);
          return campaignData;
      }
      return null;
  }
  async getCredential() {
      ...
      const cred:any = await mysqlclient.executeQuery(sql);
  ...
  async saveCredential(obj) {
      ...
      const cred:any = await mysqlclient.executeQuery(sqlGet);
      if (cred != null && cred.length > 0 && cred[0] != null) {
          let credRec = cred[0];
          var sql = "UPDATE sf_credentials SET user_name=?,password=?,token=?, login_url=? WHERE id=?";
          try {
              const data:any = await mysqlclient.executeQuery(sql, [obj.user_name, obj.password, obj.token, obj.login_url, credRec.id]);
          ...
          try {
              const data:any = await mysqlclient.executeQuery(sql, [obj.user_name, obj.password, obj.token, obj.login_url]);
  ...
  async getDbs() {
     await mysqlclient.getMysqlConnection();
      ...
      const db:any = await mysqlclient.executeQuery(sql);
  ...
  async getTablesFromDB(dbName, campaignId) {
     ...
     await mysqlclient.getMysqlConnection();
      ...
      const tables:any = await mysqlclient.executeQuery(sql);
  ...
  async getColumsInTableFromDB(dbName, tableName, campaignId) {
     ...
     await mysqlclient.getMysqlConnection();
      ...
      const columns:any = await mysqlclient.executeQuery(sql);
  ...
  async generateData(campaignId, object, requestObj) {
    ...
    if (reqConditions && reqConditions.length > 0) {
        ...
        for (let i = 0; i < conditions.length; i++) {
            ...
            conditionQuery.push(`${table.alias}.${field.name} ${clause}`);
        }
        if (conditionError) {
            return {
    ...
    sqlScripts.push(query);
    try {
        await mysqlclient.getMysqlConnection();
        const objects = await this.convertObjectSetToArray(await mysqlclient.executeQuery(query));

        if (objects != null && objects.length > 0) {
            var errors = [];
            let resultSet = objects;
            if (resultSet) {
            ...
            if (errors && errors.length > 0) {
                return { result: objects,sql:sqlScripts, errors: errors };
            }
            // build subQueries
            var subObects = await this.getSubObjects(campaignId, object);
            if (subObects && subObects.length > 0) {
                ...
                for (let j = 0; j < resultSet.length; j++) {
                    ...
                    for (let i = 0; i < subObects.length; i++) {
                        ...
                        if (conditions && conditions.length > 0) {
                            for (let i = 0; i < conditions.length; i++) {
                                ...
                                for (let k = 0; k < condition.fields.length; k++) {
                                    ...
                                    fieldsMapping.push(`${subTable.alias}.${subField.name} = '${data[parentField.default_sf_map_name]}'`);
                                }
                        ...
                        await mysqlclient.getMysqlConnection();
                        sqlScripts.push(query);
                        const subObjects:any = await mysqlclient.executeQuery(query);
                        if (subObjects != null && subObjects.length > 0) {
                            var errors = [];
                            let subQueryResult = await this.convertObjectSetToArray(subObjects);
                            if (subQueryResult) {
                            ...
                            var subObjectData = {[subObects[i].name]:[]}
                            subObjectData[subObects[i].name] = [...subQueryResult];
                            resultSet[j] = {...resultSet[j], ...subObjectData};
                        }
                    }
                }
            }
        ...
        return { result: objects ,sql:sqlScripts};
    }
    catch (err) {
        return {
            errorMessage: `error in executing the query,${err}`
  ...
  async constructQuery(fields, tables, campaignId, object, subObject, validations, transformations) {
      ...
      for (let i = 0; i < fields.length; i++) {
          ...
          var fieldName = (field.function  && field.function != 'null')? `${field.function}(${table.alias}.${field.name})` : `${table.alias}.${field.name}`;
  ...
  async getObjects(campaignId) {
     await mysqlclient.getMysqlConnection();
      let sql = `select * from sf_dataset_objects where campaign_id ${campaignId ? ' =' + campaignId : ' IS NULL'}`;
      const objects:any = await mysqlclient.executeQuery(sql);
      if (objects != null && objects.length > 0) {
          return objects;
      }
      return null;
  }
  async getSubObjects(campaignId, object) {
  ...
  async getSubObjectsConditions(campaignId, objectId, subobjectId) {
      var dataset :any = await this.getDataSet(campaignId, false);
  ...
  async getSubObjectsByObjectId(objId) {
     await mysqlclient.getMysqlConnection();
  ...
  async getRelations(campaignId, object, subobject = null) {
      var dataset :any = await this.getDataSet(campaignId, false);
  ...
  async getConditions(campaignId, object) {
      var dataset :any = await this.getDataSet(campaignId, false);
      if (dataset) {
  ...
  async getTables(campaignId, object, subObject = null) {
     await mysqlclient.getMysqlConnection();
      ...
      try {
         await mysqlclient.getMysqlConnection();
          ...
          if (data[0] != null && data[0].affectedRows > 0) {
              ...
              if (validatorValues) {
                  try {
                      await mysqlclient.executeQuery(sqlValidationParams, [validatorValues])
                  }
                  catch (err) {
  ...
  async updateValidations(campaignId, validation, validationId) {
      ...
      try {
         await mysqlclient.getMysqlConnection();
          const validator:any = await mysqlclient.executeQuery(sqlgetValidator);
          if (validator != null && validator.length > 0 && validator[0] != null) {
              ...
              if (validatorRec.type != validation.type) {
                  ...
                  const fields:any = await mysqlclient.executeQuery(sqlGetFields);
                  if (fields != null && fields.length > 0) {
                      let fieldsRecrds = fields;
      ...
      try {
         await mysqlclient.getMysqlConnection();
          await mysqlclient.executeQuery(sql);
          return { success: true };
      }
      catch (err) {
          return {
  ...
  async deleteTransformation(transformationId) {
      ...
      try {
         await mysqlclient.getMysqlConnection();
          const transformations:any = await mysqlclient.executeQuery(sqlgetTransformer);
         ...
         await mysqlclient.getMysqlConnection();
          ...
          if (data[0] != null && data[0].affectedRows > 0) {
              ...
              if (transformationValues.length > 0) {
                  try {
                     await mysqlclient.getMysqlConnection();
                      await mysqlclient.executeQuery(sqlValidationParams, [transformationValues]);
                  }
                  catch (err) {
                      if (err.code != "ER_DUP_ENTRY") {
                          throw err;
  ...
  async createOrUpdateFields(campaignId, fields, id) {
     ...
     await mysqlclient.getMysqlConnection();
      const data:any = await mysqlclient.executeQuery(sql, [fields.table, fields.name, fields.object]);
  ...
  async UpdateFields(campaignId, field, id) {
      ...
      try {
         ...
         await mysqlclient.getMysqlConnection();
          ...
          const data:any = await mysqlclient.executeQuery(sql, [`${field.table}`, field.function, `${field.object}`, field.sub_object, `${field.alias_name}`, `${field.name}`, `${field.title}`, `${field.default_sf_map_name}`, `${field.type}`, sendToSf, id]);
          if (data[0] != null && data[0].affectedRows > 0) {
              ...
              if (validations.length > 0) {
                  ...
                  if (validatorRecParams.length > 0) {
                      try {
                         await mysqlclient.getMysqlConnection();
                          var validationRecResults:any = await mysqlclient.executeQuery(sqlValidationParams, [validatorRecParams]);
                          if (validationRecResults != null && validationRecResults.length > 0) {
                              var validationParamFields = validationRecResults[0];
                              ...
                              if (validationRecRowCount > 0) {
                                          ...
                                          for (let k = 0; k < validationRec.length; k++) {
                                              ...
                                              if (params != null && params.length > 0) {
                                                  for (let j = 0; j < params.length; j++) {
                                                      ...
                                                      try {
                                                         await mysqlclient.getMysqlConnection();
                                                          const validatorParams:any = await mysqlclient.executeQuery(sqlgetValidatorParam, [validationParamRec.name, validationParamRec.type, dataSetValidationRecId]);
                                                          if (validatorParams != null && validatorParams.length > 0) {
                                                              let validatorParamsRec = validatorParams[0];
                                  ...
                                  try {
                                     await mysqlclient.getMysqlConnection();
                                      await mysqlclient.executeQuery(sqlValidationParamsValues, [validatorRecParamsValues]);
                                  }
                                  catch (err) {
                                      if (err.code != "ER_DUP_ENTRY") {
                                          throw err;
              ...
              if (transformations.length > 0 && transformations[0].length > 0) {
                  ...
                  for (let i = 0; i < transformations.length; i++) {
                      ...
                      if (transformRecrds != null && transformRecrds.length > 0) {
                          for (let j = 0; j < transformRecrds.length; j++) {
                              ...
                              try {
                                 await mysqlclient.getMysqlConnection();
                                  const transformer:any = await mysqlclient.executeQuery(sqlgetTransformer, [transformRec.name, transformRec.method]);
                                  if (transformer != null && transformer.length > 0) {
                                      let transformerRec = transformer[0];
                  ...
                  if (transformerRecParams.length > 0) {
                      try {
                         await mysqlclient.getMysqlConnection();
                          await mysqlclient.executeQuery(sqltransformationParams, [transformerRecParams]);
                      }
                      catch (err) {
                          if (err.code != "ER_DUP_ENTRY") {
                              throw err;
                  ...
                  for (let i = 0; i < validations.length; i++) {
                      ...
                      if (validationRecrds != null && validationRecrds.length > 0) {
                          for (let j = 0; j < validationRecrds.length; j++) {
                              ...
                              try {
                                 await mysqlclient.getMysqlConnection();
                                  const validator:any = await mysqlclient.executeQuery(sqlgetValidator, [validationRec.name, validationRec.type]);
                                  if (validator != null && validator.length > 0) {
                                      let validatorRec = validator[0];
  ...
  async getValidations(campaignId) {
    ...
    from sf_validations v WHERE v.campaign_id${campaignId ? ' =' + campaignId : ' IS NULL'}`;
     await mysqlclient.getMysqlConnection();
      const validationResult:any = await mysqlclient.executeQuery(sql);
  ...
  async getTransformations(campaignId) {
    ...
    from sf_transformations t WHERE t.campaign_id${campaignId ? ' =' + campaignId : ' IS NULL'}`;
     await mysqlclient.getMysqlConnection();
      const transformationResult:any = await mysqlclient.executeQuery(sql);
  ...
  async createOrUpdateTable(campaignId, obj, tableId) {
      ...
      if (obj.is_main_table) {
          ...
          try {
             await mysqlclient.getMysqlConnection();
              const tables:any = await mysqlclient.executeQuery(sqlgetUsed);
              if (tables != null && tables.length > 0) {
                  return {
  ...
  async updateSubConditions(campaignId, condition, id) {
      ...
      try {
         ...
         await mysqlclient.getMysqlConnection();
          ...
          const data:any = await mysqlclient.executeQuery(sql, arrValues);
          if (data[0] != null && data[0].affectedRows > 0) {
  ...
  async deleteTable(tableId) {
      ...
      try {
         await mysqlclient.getMysqlConnection();
          const fields:any = await mysqlclient.executeQuery(sqlgetrelations);
          if (fields != null && fields.length > 0) {
              let fieldsRecrds = fields;
              if (fieldsRecrds.length > 0) {
                  return {
          ...
          const fieldsConditions:any = await mysqlclient.executeQuery(sqlgetconditions);
          if (fieldsConditions != null && fieldsConditions.length > 0) {
              let fieldsRecrds = fieldsConditions;
              if (fieldsRecrds.length > 0) {
                  return {
          ...
          const sfFields:any = await mysqlclient.executeQuery(sqlgetFields);
          if (sfFields != null && sfFields.length > 0) {
              let fieldsRecrds = sfFields;
              if (fieldsRecrds.length > 0) {
                  var fieldString = "<ul>";
                  for (let i = 0; i < fieldsRecrds.length; i++) {
                      fieldString += "<li key='" + fieldsRecrds[i].id + "'>" + fieldsRecrds[i].name + " - <b>" + fieldsRecrds[i].object + "</b></li>";
  ...
  async updateObject(campaignId, obj, id) {
     ...
     await mysqlclient.getMysqlConnection();
      const data:any = await mysqlclient.executeQuery(sql, [`${obj.name}`, `${obj.display_name}`, `${obj.api_endpoint}`, `${obj.sqs_topic_arn}`, `${obj.key_field}`, campaignId, id]);
      if (data[0] != null && data[0].affectedRows > 0) {
          return true;
      }
      else {
  ...
  async updateSubObject(campaignId, obj, id) {
      ...
      var sql = `UPDATE sf_dataset_sub_objects 
              ...
              WHERE id=?`;
     await mysqlclient.getMysqlConnection();
      const data:any = await mysqlclient.executeQuery(sql, [`${obj.name}`, `${obj.object}`, `${apply_on_result}`, id]);
      if (data[0] != null && data[0].affectedRows > 0) {
          return true;
      }
      else {
  ...
  async updateTableRelations(campaignId, relation, id) {
      ...
      try {
         await mysqlclient.getMysqlConnection();
          const data:any = await mysqlclient.executeQuery(sql, [`${fieldParentTable}`, fieldSubObject, fieldTargetTable, `${relation.relation}`, fieldParentField, fieldTargetField, relation.parent_exp, targetIsConstant, relation.target_table_exp, relation.target_exp, `${relation.additional_join_condition}`, campaignId, id]);
          if (data[0] != null && data[0].affectedRows > 0) {
              return true;
          }
      }
      ...
      if (subConditions.length > 0) {
          ...
          for (let i = 0; i < subConditions.length; i++) {
            ...
            where_clause,groupby_clause,campaign_id) VALUES ?`;
                 ...
                 await mysqlclient.getMysqlConnection();
                  const data:any = await mysqlclient.executeQuery(sql, [conditionValues]);
                  if (data[0] != null && data[0].affectedRows > 0) {
  ...
  async getFields(campaignId, object, table = null, subObject = null) {
      var dataset :any = await this.getDataSet(campaignId, false);
  ...
  async deleteField(fieldId, deleteField = true) {
      try {
         await mysqlclient.getMysqlConnection();
          ...
          await mysqlclient.executeQuery(sql, [fieldId, deleteField && deleteField === true ? 1 : 0]);
          return { success: true };
      }
      catch (err) {
          return {
  ...
  async convertObjectSetToArray(objects){
      var array = [];
  ...
  async getDataSet(campaignId, trimIds = true) {
     await mysqlclient.getMysqlConnection();
      var sql = "CALL getDataSetConfiguration(?)";
      const datasetResult:any = await mysqlclient.executeQuery(sql, [campaignId ? campaignId : null]);
      var datasetJson = { objects: [], validations: [], transformations: [], fields: [] ,subobjects:[],tables:[],
        conditions:[], relations :[],sub_conditions:[]};
      if (datasetResult != null && datasetResult.length > 0) {
          var validations = await this.convertObjectSetToArray(datasetResult[0]);
          var transformations = await this.convertObjectSetToArray(datasetResult[1]);
          var fields = await this.convertObjectSetToArray(datasetResult[2]);
          var objects = await this.convertObjectSetToArray(datasetResult[3]);
          var tables = await this.convertObjectSetToArray(datasetResult[4]);
          var relations = await this.convertObjectSetToArray(datasetResult[5]);
          var conditions = await this.convertObjectSetToArray(datasetResult[6]);
          var subObjects = await this.convertObjectSetToArray(datasetResult[7]);
          var subObjectConditions = await this.convertObjectSetToArray(datasetResult[8]);
  ...
  async constructDataSet(dataset, validations, transformations) {
      var fieldsArr = [];
      if (dataset) {
  ...
  async constructUserDataFromUser(campaigns) {
      var currenYear = new Date().getFullYear();
      const campaignArr = new Array();
      ...
      if (campaigns) {
          campaigns.forEach(campaignRec => {
              let campaign = this.findCampaignDataFromArray(campaignArr, campaignRec.id);
              if (!campaign) {
  ...
  findCampaignDataFromArray(campaignArray, id) {
