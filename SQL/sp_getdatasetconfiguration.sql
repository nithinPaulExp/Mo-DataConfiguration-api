...
SET fieldsSQL = ' select df.id as field_id, df.sql_function,df.table_id as field_table,df.name as field_name,df.title as field_title,df.sf_map_name as field_sf_map, df.type as field_type,df.object,df.sub_object_id,obj.display_name as object_name,df.alias_name,df.send_to_sf,tbl.table_name,
...
SET tableSQL = ' SELECT id,object_id,sub_object_id, table_db, table_name, alias,is_main_table FROM sf_dataset_tables WHERE ';
...
SET conditionSQL = ' SELECT id,name,object_id, table_id,where_field, group_by,where_clause FROM sf_dataset_object_conditions WHERE ';
...
SET subObjectsSQL = ' SELECT id,name, object_id,apply_on_result from sf_dataset_sub_objects WHERE ';

if campaignId IS NULL then
 SET subObjectsSQL = CONCAT(subObjectsSQL,'campaign_id is null');
 else 
 SET subObjectsSQL = CONCAT(subObjectsSQL,'campaign_id=',campaignId);
end if;
SET @sqlQuery = subObjectsSQL;
PREPARE stmt FROM @sqlQuery;
	EXECUTE stmt;     
	DEALLOCATE PREPARE stmt;
	
SET subconditionSQL = ' SELECT sd.id as id ,sd.name,
								sd.apply_table,sd.apply_table_field,sd.field_where_clause,
								object_id,sub_object_id,apply_is_data,apply_is_data_conjunction_type,where_clause,groupby_clause,
								parent_table_id,sub_table_id,parent_table_field,sub_table_field
								 FROM sf_dataset_sub_object_conditions sd LEFT JOIN sf_dataset_sub_object_conditions_fields sdf ON sd.id = sdf.sub_condition_id WHERE ';

if campaignId IS NULL then
 SET subconditionSQL = CONCAT(subconditionSQL,'sd.campaign_id is null');
 else 
 SET subconditionSQL = CONCAT(subconditionSQL,'sd.campaign_id=',campaignId);
end if;
SET @sqlQuery = subconditionSQL;
PREPARE stmt FROM @sqlQuery;
	EXECUTE stmt;     
	DEALLOCATE PREPARE stmt;
