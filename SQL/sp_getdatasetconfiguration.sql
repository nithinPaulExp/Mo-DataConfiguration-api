...
SET fieldsSQL = ' select df.id as field_id, df.table_name as field_table,df.name as field_name,df.title as field_title,df.sf_map_name as field_sf_map, df.type as field_type,df.object,obj.display_name as object_name,df.alias_name,df.join_type,df.join_table,df.join_parent_column,df.join_target_column,
...
SET objectsSQL = ' SELECT id,name, display_name, api_endpoint, sqs_topic_arn from sf_dataset_objects WHERE ';
